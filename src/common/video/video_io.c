...
video_reader_t* video_reader_alloc_init(const char* path, const size_t start, const size_t end, const size_t skip,
    ...
    switch (codec_type) {
        case VCDC_FFMPEG_IO: {
#ifdef MOTION_USE_FFMPEG_IO
            return video_reader_ffio_alloc_init(path, start, end, skip, bufferize, n_ffmpeg_threads, hwaccel,
                                                pixfmt, ffmpeg_debug, ffmpeg_in_extra_opts, i0, i1, j0, j1);
            break;
#else
