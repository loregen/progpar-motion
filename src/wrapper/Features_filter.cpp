...
: spu::module::Stateful(), i0(i0), i1(i1), j0(j0), j1(j1), max_size_f(max_size_f), min_size_f(min_size_f), max_size_roi(max_size_roi), max_size_roi2(max_size_roi2)
{
    ...
    this->create_codelet(c, [si_labels, so_labels, so_roi, so_roi_tmp, si_n_RoIs, max_size_roi2]
        ...
        uint32_t n_RoIs0 = features_filter_surface(in_labels, out_labels, ff.i0, ff.i1, ff.j0, ff.j1, RoIs_tmp, *n_RoIs, ff.min_size_f, ff.max_size_f);
        assert(n_RoIs0 <= (uint32_t)max_size_roi2);
